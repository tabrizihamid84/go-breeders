...
func (app *application) render(w http.ResponseWriter, t string, td *templateData) {
	...
	// if we are using the template cache, try to get the template from our map, stored in the recevier
	...
	if tmpl == nil {
		newTemplate, err := app.buildTemplateFromSidk(t)
	...
	if err := tmpl.ExecuteTemplate(w, t, td); err != nil {
		log.Println("Error excuting template: ", err)
		http.Error(w, err.Error(), http.StatusInternalServerError)
	}
}

func (app *application) buildTemplateFromSidk(t string) (*template.Template, error) {
